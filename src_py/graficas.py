...
from consts import MI_COLUMNS, VARK_COLUMNS, IM_DISPLAY_LABELS, VARK_DISPLAY_LABELS
...
def info_graphics_fromDataframe(df: pl.DataFrame, selected_id: int | None = None, view_mode: str = "person",) -> dict[str, Any]:
    ...
    return {
        ...
        "labels": { #para graficas de RADAR
            "im": [IM_DISPLAY_LABELS[label] for label in MI_COLUMNS], #lista de etiquetas para la grafica de radar de inteligencias múltiples
...
def _draw_histogram_chart(ax: plt.Axes, df:pl.DataFrame) -> None:
    ...
    values =  [_normalize_cronotipo(value) for value in df["Chronotype"].to_list()]
...
if __name__ == "__main__":
    from main import load_preprocessed_lf
    df = load_preprocessed_lf(Path("data/test_data/preprocessed_test.parquet")).collect()
